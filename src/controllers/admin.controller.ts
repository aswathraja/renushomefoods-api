...
export class AdminController {
	...
	async getAllAdCampaigns(
		...
		try {
			...
			const { name, fromStartDate, toStartDate, fromEndDate, toEndDate, subject, message, users } =
			...
			if (fromStartDate) {
				adjustedFromStartDate = new Date(`${fromStartDate}T00:00:00.000Z`)
					.toISOString()
					.slice(0, 19)
					.replace('T', ' ');
			}
	...
	async saveOrUpdateAdCampaign(
		...
		try {
			...
			const { id, name, startDate, endDate, message, subject, userIds, imagePath } = decryptedBody;

			if (!name || !startDate || !endDate || !message || !subject || !Array.isArray(userIds)) {
				throw new HttpException(
					{
						error: encryptPayload({
			...
			if (id) {
				...
				if (file) {
					...
					// Save new file
					const savedImagePath = await saveFile(file, 'ads', adCampaign.toJSON()?.id);
				...
				const existingUserIds = existingCampaignUsers.map((acu) => acu.toJSON().userId);
				...
				if (file) {
					...
					await AdCampaign.update({ imagePath: savedImagePath }, { where: { id: adCampaign.id } });
				}

				// Deduplicate userIds before creating
				const uniqueUserIdsForCreate = Array.from(new Set(userIds));

				const recordsToCreate = uniqueUserIdsForCreate.map((userId) => ({
			...
			return {
				response: encryptPayload({
					id: adCampaign.id,
					message: id ? 'AdCampaign updated successfully.' : 'AdCampaign created successfully.',
				}),
			};
		} catch (error) {
			...
			throw new HttpException(
				{
					error: encryptPayload({
						error: `Failed to save or update AdCampaign. ${errorMessage}`,
					}),
				},
				HttpStatus.INTERNAL_SERVER_ERROR,
			);
		...
		try {
			...
			if (adCampaign.toJSON().imagePath) {
				try {
					...
					logger.error(new Error(`Failed to delete ad campaign image file: ${err.message}`));
				}
			}
	...
	async getUserDetails(
		...
		try {
			...
			if ((email && user.toJSON().email !== email) || (phone && user.toJSON().phone !== phone)) {
				throw new HttpException(
					{
						error: encryptPayload({
	...
	async getDashboardKPIs(
		...
		try {
			...
			const { fromDate, toDate, orderStatus, category, product, name, phone } = decryptPayload(
				body.request,
			);
			...
			const response = {
				totalSalesCollected: kpis.totalSalesCollected || 0,
				avgOrderValue: kpis.totalOrders > 0 ? kpis.totalSales / kpis.totalOrders : 0,
		...
		try {
			...
			for (const chartType of chartTypes) {
				...
				if (chartType === 'Products with Quantities by Order Status') {
					...
					(results as any[]).forEach((row: any) => {
						const product = row.product || row.Product || row.productName || row.ProductName;
						...
						productMap[product].pendingQuantity += Number(row.pendingQuantity) ?? 0;
						productMap[product].fulfilledQuantity += Number(row.fulfilledQuantity) ?? 0;
					});

					processedResults = Object.entries(productMap).map(([product, quantities]) => ({
						product,
					...
					processedResults = (processedResults as any[]).map((row: any) => {
						const category = row.category || row.product || row.orderStatus || '';
						...
						return {
							...row,
							displayLabel: `${category} : ${INRFormater.format(sanitizeStringToNumber(totalSales))}`,
						};
					});
				}
	...
	@Post('get-orders')
	async getOrders(@Body() body: { request: string }, @Headers() headers: Record<string, string>) {
		try {
			// Authenticate admin
			const authHeader = headers.authorization || '';
			await this.authenticateAdmin(authHeader);
			...
			orders.forEach((order: any) => {
				const orderTotal = order.products.reduce(
					(sum: number, product: any) => sum + (product?.productTotal || 0),
					0,
				);
				const productDiscount = order.products.reduce(
					(sum: number, product: any) => sum + (product?.productDiscount || 0),
					0,
				);
	...
	async getUsersWithDefaultAddress(
		...
		try {
			...
			throw new HttpException(
				{
					error: encryptPayload({
						error: `Failed to fetch users with default address. ${errorMessage}`,
					}),
				},
				HttpStatus.INTERNAL_SERVER_ERROR,
			);
	...
	async getUserAddresses(
		...
		try {
			...
			const addresses = await UserAddress.findAll({
				where: { userId },
				attributes: ['id', 'name', 'addressLine1', 'city', 'state', 'country', 'pincode', 'phone'],
			});
			return {
	...
	@Post('save-order')
	async saveOrder(@Body() body: { request: string }, @Headers() headers: Record<string, string>) {
		try {
			// Authenticate admin
			const authHeader = headers.authorization || '';
			await this.authenticateAdmin(authHeader);
			...
			if (Boolean(updateOrderStatus) === true) {
				...
				const encryptedResponse = {
					response: encryptPayload({
						...
						cartId: order.toJSON().cartId,
						userAddressId: order.toJSON().userAddressId,
			...
			if (!user) {
				...
				user = await User.create({
					...
					email:
						customerEmail ||
						`${customerName.toLowerCase().replace(/\s+/gim, '')}@renushomefoods.com`, // Assuming email not provided
					phone: customerPhone,
					password: '',
					otp,
				});
			...
			for (const prod of products) {
				...
				const priceList = dbProduct.PriceLists.find((pl: any) => pl.id === prod.priceListId);
				if (!priceList) {
					throw new HttpException(
						{
							error: encryptPayload({
				...
				const existing = existingCartProducts.find(
					(cp: any) => cp.productId === dbProduct.id && cp.priceListId === prod.priceListId,
				);
				if (existing) {
			...
			for (const existing of existingCartProducts) {
				if (productIdsInRequest.indexOf(existing.toJSON().Product?.name) === -1) {
			...
			if (orderId) {
				...
				isOrderShipped = order.toJSON().status !== orderStatus && orderStatus === 'Shipped';
				isOrderDelivered = order.toJSON().status !== orderStatus && orderStatus === 'Delivered';
				const isOrderCancelled =
					order.toJSON().status !== orderStatus && orderStatus === 'Cancelled';
				await order.update({
					userId: user.id,
	...
	async createCoupon(
		...
		try {
			...
			if (!code || !startDate || !endDate || !discounts || !productIds || !userIds) {
				throw new HttpException(
					{
						error: encryptPayload({
							error:
								'Missing required fields: code, startDate, endDate, discounts, productIds, userIds.',
						}),
					},
					HttpStatus.BAD_REQUEST,
				);
			...
			if (couponId) {
				...
				await couponCode.update({
					...
					isForNewUsers: isForNewUsers !== undefined ? isForNewUsers : false,
					isForAllUsers: isForAllUsers !== undefined ? isForAllUsers : false,
				});
				isUpdate = true;
			} else {
			...
			userIdsToCreate = [...new Set([...userIdsToCreate, ...adminUserIds])];
			...
			if (isUpdate) {
				...
				for (const discount of discounts) {
					const existing = existingDiscounts.find((ed: any) => ed.toJSON().name === discount.name);
					if (existing) {
						await existing.update({
							discount: discount.discount,
							flatrate: discount.flatrate !== undefined ? discount.flatrate : false,
						});
					} else {
				...
				for (const existing of existingDiscounts) {
					if (!discountNamesInRequest.includes(existing.toJSON().name)) {
						await existing.destroy();
					}
				}
				...
				const existingProductIds = existingProducts.map((ep: any) => ep.toJSON().productId);
				...
				for (const existing of existingProducts) {
					if (!productIdsToCreate.includes(existing.toJSON().productId)) {
						await existing.destroy();
					}
				}
				...
				const existingUserIds = existingUsers.map((eu: any) => eu.toJSON().userId);
			...
			return {
				response: encryptPayload({
					...
					message: isUpdate ? 'Coupon updated successfully.' : 'Coupon created successfully.',
				}),
			};
		} catch (error) {
	...
	@Post('save-user')
	async saveUser(@Body() body: { request: string }, @Headers() headers: Record<string, string>) {
		...
		try {
			...
			if (userInstance) {
				...
				const passwordToSave =
					user.hasOwnProperty('password') && Boolean(user?.password) === true
				...
				// Create new user
				const passwordToSave =
					user.hasOwnProperty('password') && user.password !== undefined
						? hashPassword(user.password)
						: '';
			...
			if (Array.isArray(user.roles)) {
				...
				const currentRoleIds = currentUserRoles.map((ur) => ur.toJSON().roleId);
				const requestedRoleIds = user.roles;

				// Roles to add
				const rolesToAdd = requestedRoleIds.filter((id) => !currentRoleIds.includes(id));
				// Roles to remove
				const rolesToRemove = currentRoleIds.filter((id) => !requestedRoleIds.includes(id));
			...
			if (user.resetPassword === true && Boolean(user?.password) === false) {
				const otp = this.appService.generateRandomNumber(4);
				await userInstance.update({ otp }, { transaction });
				await this.appService.sendMail({
					to: userInstance.toJSON().email,
						...
						await existingAddress.update(
							{
								...
								phone: addr.phone,
								isDefault:
									addr.isDefault !== undefined ? addr.isDefault : existingAddress.isDefault,
							},
							{ transaction },
						);
					} else {
			...
			return {
				response: encryptPayload({
					userId,
					message: 'User and addresses saved or updated successfully.',
				}),
			};
		} catch (error) {
	...
	async sendAdCampaignEmails(@Body() body: { request: string }) {
		try {
			...
			const { subject, message, imagePath, imageURL } = adCampaign.toJSON();
			...
			throw new HttpException(
				{
					error: encryptPayload({
						error: `Failed to send ad campaign emails. ${errorMessage}`,
					}),
				},
				HttpStatus.INTERNAL_SERVER_ERROR,
			);
	...
	async downloadDashboard(
		...
		try {
			...
			const dashboardType = (filters.dashboardType as 'Sales' | 'Inventory') || 'Sales';
			if (!['Sales', 'Inventory'].includes(dashboardType)) {
				throw new HttpException(
					{
						error: encryptPayload({
							error: "Invalid dashboardType. Must be 'Sales' or 'Inventory'",
						}),
					},
					HttpStatus.BAD_REQUEST,
				);
			...
			const buffer = await this.pdfService.generateDashboardPdf(filters, format, dashboardType);

			// FIXED: Add Content-Length + CORS for frontend
	...
	async exportExcelData(
		...
		try {
			...
			// Decrypt request to extract filters
			const filters = decryptPayload(body.request);
			const { fromDate, toDate, orderStatus, category, product, name, phone } = filters;
			...
			orders.forEach((order: any) => {
				const productsText = order.products
					.map((p: any) => `${p.productName} (${p.quantity}x ₹${p.price})`)
					.join(', ');
				...
				ordersSheet.addRow({
					...
					expectedDeliveryDate: new Date(order.expectedDeliveryDate).toLocaleDateString(),
			...
			// Add chart data
			Object.entries(chartData).forEach(([chartKey, chartValues]: [string, any]) => {
				if (Array.isArray(chartValues)) {
					const totalChartValue = chartValues.reduce((total: number, item: any) => {
						if (item.totalSales) {
					...
					chartValues.forEach((value: any) => {
						if (chartKey === 'productswithquantitiesbyorderstatus') {
							const totalQuantity = value.pendingQuantity + value.fulfilledQuantity;
							const pendingPercent = (value.pendingQuantity / totalQuantity) * 100;
							const fulfilledPercent = (value.fulfilledQuantity / totalQuantity) * 100;
							...
							const category = value.category || value.product || value.name || value.orderStatus;
							const chartValue = value.totalSales || value.quantity || value.count || 0;
							chartSheet.addRow({
								chartType: chartTypeKeyMap[chartKey],
								category,
								value: typeof chartValue === 'number' ? `₹${chartValue.toFixed(2)}` : chartValue,
								percentage:
									chartValue && totalChartValue
										? `${((chartValue / totalChartValue) * 100).toFixed(2)}%`
										: `${0}%`,
							});
						}
					});
