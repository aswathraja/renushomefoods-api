...
export class UserController {
	...
	async validateUser(@Body() body: { request: string }) {
		try {
			...
			const normalizePhone = (num: string) => num.replace(/^\+\d{1,3}|\s+/g, '');
	...
	async checkAvailability(@Body() body: { request?: string }) {
		...
		try {
			...
			for (const user of existingUsers) {
				...
				const dbPhoneNormalized = userJSON.phone.replace(/^\+\d{1,2}|\s+/g, '');
	...
	async forgotPassword(@Body() body: { request?: string }) {
		try {
			...
			throw new HttpException(
				{
					error: encryptPayload({
						error: `Failed to process forgot password request. ${error.message}`,
					}),
				},
				HttpStatus.INTERNAL_SERVER_ERROR,
			);
	...
	async login(@Body() body: { request: string }) {
		try {
			const decryptedBody = decryptPayload(body.request);
			const identifierDigits = decryptedBody.identifier.replace(/\D/g, '').slice(-10);
			...
			if (user.toJSON().password === '') {
				...
				const maskedEmail = this.appService.maskEmail(updatedUser.toJSON().email);
				return {
					response: encryptPayload({
						message: `OTP has been sent to your registered email id : ${maskedEmail}.`,
					}),
			...
			const valid = comparePassword(decryptedBody.password, user.toJSON().password);
			if (!valid) {
				throw new HttpException(
					{
						error: encryptPayload({
	...
	async verifyToken(@Body() body: { request: string }) {
		try {
			...
			const session = await UserSession.findOne({
				where: {
					[Op.or]: [{ token: decryptedToken }, { prevToken: decryptedToken }],
					isExpired: false,
				},
			});
			...
			if (new Date() > session.expiry) {
				...
				await session.update({ isExpired: true });
				throw new HttpException({ error: 'Session expired' }, HttpStatus.FORBIDDEN);
			}
	...
	async createOrUpdateAddress(@Body() body: { request: string }) {
		try {
			...
			const { addressId, name, addressLine1, city, state, country, pincode, phone, isDefault } =
				decryptedBody;
			...
			if (addressId) {
				...
				if (isDefault) {
					...
					await UserAddress.update({ isDefault: false }, { where: { userId, isDefault: true } });
				}
			...
			throw new HttpException(
				{
					error: encryptPayload({
						error: `Failed to create or update address. ${error?.message}`,
					}),
				},
				HttpStatus.INTERNAL_SERVER_ERROR,
			);
	...
	async changePassword(@Body() body: { request: string }) {
		try {
			...
			const valid = comparePassword(currentPassword, user.toJSON().password);
			if (!valid) {
				throw new HttpException(
					{
						error: encryptPayload({
				...
				throw new HttpException(
					{
						error: encryptPayload({
							error:
								'Cannot delete the only address. Please add another address before deleting this one.',
						}),
					},
					HttpStatus.BAD_REQUEST,
				);
