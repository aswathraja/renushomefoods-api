...
export class AppService {
	...
	public async getOrderInvoiceData(orderId: string, message: string) {
		try {
			...
			const items = order.toJSON().Cart.CartProducts.map((cartProduct) => ({
				name: cartProduct.Product?.name,
				quantity: cartProduct.quantity,
				weight: cartProduct.PriceList?.weight || 'N/A',
				price: cartProduct.PriceList.unitprice.toFixed(2),
				total: (cartProduct.quantity * cartProduct.PriceList.unitprice).toFixed(2),
			}));
				...
				couponDiscounts.forEach((discount) => {
					...
					if (applicableProductIds.length === 0) {
						...
						const applicableSubtotal = order
							.toJSON()
							.Cart.CartProducts.filter((cp) => applicableProductIds.includes(cp.productId))
							.reduce(
								(sum, cartProduct) => sum + cartProduct.quantity * cartProduct.PriceList.unitprice,
								0,
							);
			...
			if (
				...
				const couponDiscounts = orderCoupons[0].CouponCode.CouponDiscounts;
				shippingDiscountObj = couponDiscounts.find((d) => d.name === 'Shipping');
			}
			...
			// Calculate total
			const total = adjustedSubtotal + this.sanitizeStringToNumber(shipping);
			return {
				...
				shippingPhone:
					order.toJSON().shippingMethod === 'Home Delivery' ? userPhone : '+91 93637-20792',
				items,
				...
				orderDate: this.formatDate(new Date(order.toJSON().orderedDate), true),
				pan: '',
				gst: '',
				cin: '',
				...
				couponCode: order.toJSON().OrderCoupons?.[0]?.CouponCode?.code || '',
				totalDiscount: totalDiscount > 0 ? totalDiscount.toFixed(2) : undefined,
				orderNotes: order.toJSON().notes?.replace(/\n/gim, '<br/>') || '',
			};
		} catch (error) {
	...
	private renderTemplate(template: string, data: any): string {
		const templatePath = join(resolve(__dirname), '../', 'templates', `${template}.html`);
	...
	public async sendMail({
		...
		try {
			...
			const fromAddress =
				template === 'order-invoice' ? process.env.ORDERS_EMAIL : process.env.CONTACT_EMAIL;
