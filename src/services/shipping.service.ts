...
export class ShippingService {
	 ...
	 * Free Store Pickup: ₹0
	 */
	calculateShippingCost(pincode: string, weightInGrams: number, shippingMethod: string): number {
	...
	calculateShippingDiscount(
		...
		if (couponDiscount.flatrate) {
			...
			const discountAmount = baseShippingCost * (couponDiscount.discount / 100);
	...
	calculateFinalShipping(
		...
		const baseCost = this.calculateShippingCost(pincode, weightInGrams, shippingMethod);
		...
		// Calculate discount
		const discount = this.calculateShippingDiscount(couponDiscount, baseCost);
	...
	getShippingDetails(
		...
		const pincodeNum = parseInt(pincode, 10);
		let zone = 'Other';
		if (!isNaN(pincodeNum) && pincodeNum >= 600000 && pincodeNum <= 600300) {
			zone = 'Chennai Local';
		}

		// Determine weight category
		const weightCategory = weightInGrams >= 500 ? 'Heavy (≥500g)' : 'Light (<500g)';
	...
	logShippingCalculation(
		...
		const details = this.getShippingDetails(pincode, weightInGrams, shippingMethod, couponDiscount);
	}
}
