	...
	new Promise((resolve, reject) => {
		...
		ffmpeg.on('close', (code) => {
			...
			try {
				...
				logger.error(new Error(`Failed to delete temp input file: ${(err as Error).message}`));
			}
		...
		ffmpeg.on('error', (err: any) => {
			...
			try {
				...
				logger.error(new Error(`Failed to delete temp input file: ${cleanupErr.message}`));
			}
...
export const saveFile = async (
		...
		throw new HttpException(
			{
				error: encryptPayload({
					error:
						'Invalid file type. Only images (JPEG, PNG, GIF, WebP, SVG) and videos (MP4, MOV, WebM, AVI) are allowed.',
				}),
			},
	...
	if (type === 'ads') {
		basePath = adsPath || '';
		returnPrefix = '/ads/';
	} else if (['inventory', 'category', 'product', 'item', 'location'].includes(type)) {
		basePath = inventoryPath || '';
		if (type === 'category') {
			returnPrefix = '/categories/';
		} else if (type === 'product') {
			returnPrefix = '/products/';
		} else if (type === 'item') {
			returnPrefix = '/items/';
		} else if (type === 'location') {
			returnPrefix = '/locations/';
		} else if (type === 'inventory') {
			returnPrefix = '/inventory/';
		}
...
export const deleteFileIfExists = (filePath: string) => {
	...
	if (
		...
		try {
			unlinkSync(path.resolve(path.join(adsPath, filePath.split('/').pop() || '')));
		} catch (err: any) {
			logger.error(new Error(`Failed to delete file ${filePath}: ${err.message}`));
		}
	}
};
 ...
 */
export const sanitizeStringToNumber = (value: string | null | undefined): number => {
